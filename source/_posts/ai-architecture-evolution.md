...
处理一个句子 [x₁, x₂, ..., xₜ]，我们需要的不是一个空间上的滑动窗口，而是一个**沿时间步依次处理、并维护内部状态**的计算单元。
...
如果遗忘门 f_t ≈ 1（网络学会了"记住"），那么梯度**几乎无损地**沿细胞状态通路回传。与 Vanilla RNN 的 W_hh · tanh'(·) 不同，这里的梯度传递是**加法性的**，不是乘法性的。
...
1. **串行性**：h_t 依赖 h_{t-1}，无法并行化。处理长度为 T 的序列，至少需要 O(T) 的串行步。
...
其中 W^Q, W^K ∈ R^(d×d_k)，W^V ∈ R^(d×d_v)。
...
1. **FFN 的扩展比**：中间层维度 d_ff 通常是 d_model 的 4 倍。这个"先扩后缩"的结构让网络可以在高维空间中做非线性变换。
